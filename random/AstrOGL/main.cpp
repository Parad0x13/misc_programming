...
struct planet{
	float x, y, z, radius, mass, r, g, b, vx, vy, vz;
}planet[10];
...
int DrawGLScene(GLvoid)									// Here's where all the drawing goes
{
	...
	{
		glPushMatrix();
		glColor3f(planet[d].r, planet[d].g, planet[d].b);
		glTranslatef((float)planet[d].x, (float)planet[d].y, (float)planet[d].z);
		glutSolidSphere(planet[d].radius, xdiv, ydiv);
		glPopMatrix();
	}
...
void initplanets()
{
	for(int d=0;d<10;d++)
	{
		planet[d].x=(float)(rand()%50);
		planet[d].y=(float)(rand()%50);
		planet[d].z=(float)(rand()%50);
		planet[d].radius=(float)(rand()%5);
		planet[d].mass=(float)(rand()%1000);
		planet[d].r=1.0f;
		planet[d].g=1.0f;
		planet[d].b=0;
...
void physics()
{
	float G, g, m2, m1, distance;
	// I'm going to use the equation off the top of my head, G=g((m2-m1)/distance^2), but I forgot what g's value is...
}
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Basic 3D 1.0",1024,768,32,fullscreen))
